...
const fmtRow = r =>
  ...
  `${r.dx >= 0 ? '+' : ''}${r.dx.toFixed(0)}/${r.dy >= 0 ? '+' : ''}${r.dy.toFixed(0)}`.padStart(
    11,
  ) +
...
function fitJournal(rgbA, rgbB, geo, opt = {}) {
  ...
  return fitPair(rgbA, rgbB, pageMask(geo.quad), piv, {
    sRange: SRANGE,
    ...opt,
    reach: reachMask(geo.quad, piv, 2.0),
  })
}
...
if (argv.includes('--pair')) {
  ...
  const a = rgbOfPng(argv[i + 1]),
    b = rgbOfPng(argv[i + 2])
  const box = (flag('--box') || `${W * 0.15},${H * 0.2},${W * 0.7},${H * 0.6}`)
    .split(',')
    .map(Number)
  const mask = boxMask(...box)
  const piv = [box[0] + box[2] / 2, box[1] + box[3] / 2]
  const r = fitPair(a, b, mask, piv, {
    reach: boxMask(box[0] - box[2] / 2, box[1] - box[3] / 2, box[2] * 2, box[3] * 2),
  })
  console.log(HEAD)
...
const chrome = spawn(
  ...
  [
    ...
    `--user-data-dir=/tmp/clipfit-${PORT}`,
    'about:blank',
  ],
  { stdio: 'ignore' },
)
...
function fitPage(rgbA, rgbB, geo, opt = {}) {
  if (!geo.art || argv.includes('--face'))
    return { r: fitJournal(rgbA, rgbB, geo, opt), win: 'face' }
  const [x, y, w, h] = geo.art
  const r = fitPair(rgbA, rgbB, boxMask(x, y, w, h), geo.centre, {
    sRange: SRANGE,
    ...opt,
    reach: boxMask(x - w / 2, y - h / 2, w * 2, h * 2),
  })
  return { r, win: 'art' }
}
...
if (argv.includes('--selftest')) {
  ...
  const CASES = [
    [22.07 + LEAD, 1.0, 0, 0],
    [22.07 + LEAD, 1.12, 40, -25],
    [26.07 + LEAD, 0.88, -30, 18],
    [39.07 + LEAD, 1.06, 12, 55],
    [4.6 + LEAD, 1.15, -20, -40],
  ]
  ...
  for (const [n, k, ddx, ddy] of INCREMENTS) {
    ...
    if (shaky(r0)) {
      mute++
      console.log(
        '                        no confident peak on the plain pair — nothing to move   SKIP',
      )
      continue
    }
    spoke++
    ...
    if (bad) fails++
    console.log(
      `                        moved x${k.toFixed(2)} ${String(ddx).padStart(4)}/${String(ddy).padStart(4)}` +
  ...
  for (const [n, k] of [
    ...
    if (bad) fails++
    console.log(
      `   ${String(n).padStart(2)}  x${k.toFixed(2)}  ` +
        fmtRow(r) +
        r.rival.toFixed(3).padStart(7) +
        `   err ${err.toFixed(2)} %` +
        (bad ? '   FAIL' : ''),
...
if (argv.includes('--solve')) {
  ...
  for (const j of jobs) {
    ...
    for (let pass = 0; pass < 5; pass++) {
      best = null
      let step = pass ? 0.4 : 2
      let lo = pass ? bestRotY - 3 : -89.8,
        hi = pass ? bestRotY + 3 : -2
    ...
    console.log(
      j.tag.padStart(6) +
        ...
        best.rotY.toFixed(2).padStart(8) +
        j.scale.toFixed(3).padStart(8) +
        cx.toFixed(1).padStart(8) +
        (chk.x1 - chk.x0).toFixed(0).padStart(9) +
        chk.x0.toFixed(0).padStart(8) +
        chk.x1.toFixed(0).padStart(8) +
...
if (argv.includes('--pose')) {
  ...
  for (const j of jobs) {
    ...
    ff([
      '-y',
      '-i',
      file2png(CLIP, j.n, `${OUT}/.clipfr-${j.tag}.png`),
      '-i',
      ours,
      '-filter_complex',
    ...
    const qx = Math.min(...geo.quad.map(p => p[0])),
      qX = Math.max(...geo.quad.map(p => p[0]))
    const qy = Math.min(...geo.quad.map(p => p[1])),
      qY = Math.max(...geo.quad.map(p => p[1]))
    console.log(
      j.tag.padStart(5) +
        t.toFixed(3).padStart(8) +
        '  ' +
        `rot ${j.rot} sc ${j.scale} cx ${j.cx} cy ${j.cy} rotY ${j.rotY} p ${j.persp}`.padEnd(40) +
        `${qx.toFixed(0)}/${qy.toFixed(0)} ${(qX - qx).toFixed(0)}x${(qY - qy).toFixed(0)}`.padStart(
          20,
        ) +
        '   corners ' +
        geo.quad.map(c => `${c[0].toFixed(0)},${c[1].toFixed(0)}`).join(' '),
    )
  }
  ...
  for (const t of list) {
    ...
    console.log(
      t.toFixed(3).padStart(7) +
        String(Math.round(t * 30)).padStart(7) +
        '  ' +
        c.rot.toFixed(2).padStart(9) +
        c.scale.toFixed(3).padStart(8) +
        c.cx.toFixed(1).padStart(7) +
        c.cy.toFixed(1).padStart(7) +
        r.v.toFixed(3).padStart(9) +
        r.rival.toFixed(3).padStart(7) +
        (shaky(r) ? '  NO CONFIDENT PEAK' : ''),
    )
  }
...
if (argv.includes('--anchor')) {
  ...
  for (const frame of frames) {
      ...
      N > 1
        ? [
            ...new Set(
              Array.from(
                { length: N },
                (_, k) => rows[Math.round((k * (rows.length - 1)) / (N - 1))],
              ),
            ),
          ]
        : rows
    ...
    console.log(
      '      t     win     size      shift(px)     rot   score  rival    ' +
    ...
    for (const m of pick) {
      ...
      if (!geo) {
        console.log(`  ${m.t.toFixed(2)}  (no journal on screen)`)
        continue
      }
      ...
      console.log(
        '  ' +
          ...
          r.rival.toFixed(3).padStart(7) +
          '    ->     ' +
          a.scale.toFixed(4).padStart(7) +
          a.cx.toFixed(2).padStart(7) +
          a.cy.toFixed(2).padStart(7) +
          a.rot.toFixed(2).padStart(7) +
          (a.shaky ? '   no peak' : ''),
      )
    }
    if (!got.length) {
    ...
    out.push(row)
    console.log(
      `  MEDIAN of ${rep.length}${firm.length >= 3 ? ' firm' : ' (no firm majority — all samples used)'}` +
        `   scale ${row.scale}  cx ${row.cx}  cy ${row.cy}  rot ${row.rot}`,
    )
    console.log(
      `  SPREAD                 size ${row.spread.scale} %  cx ${row.spread.cx} px  ` +
...
for (const { label, t } of times) {
  ...
  if (!geo) {
    console.log(label + '  (no journal on screen)')
    continue
  }
  ...
  if (!p || !Number.isFinite(p.rot)) {
    console.log('  (pose unreadable)')
    continue
  }
  console.log(
    `  ours { rot: ${p.rot.toFixed(2)}, scale: ${p.scale.toFixed(3)}, ` +
      `cx: ${p.cx.toFixed(1)}, cy: ${p.cy.toFixed(1)} }  rotX ${p.rotX.toFixed(1)} rotY ${p.rotY.toFixed(1)}`,
  )
  console.log(
    `  CLIP { rot: ${implied.rot}, scale: ${implied.scale}, ` +
